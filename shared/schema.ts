...
export const agentTypeEnum = pgEnum("agent_type", [
  ...
  // Python LitServe/LitAI Agents (2)
  "python_terminal",
  "python_sdk"
]);
...
// IPFS Project Uploads - Mastered Projects with Guaranteed 2+ Year Storage
export const publishStatusEnum = pgEnum("publish_status", ["draft", "published", "archived", "unpublished"]);
...
export const projectUploads = pgTable("project_uploads", {
  ...
  description: text("description"),
  ipfsHash: text("ipfs_hash").notNull().unique(), // CID from IPFS
  ipfsUrl: text("ipfs_url").notNull(), // ipfs:// URL
  pinataUrl: text("pinata_url"), // Pinata gateway URL
  fileSize: integer("file_size").notNull(), // Size in bytes
  pinned: boolean("pinned").default(false), // Pinning status
  pinnedAt: timestamp("pinned_at"),
  expiresAt: timestamp("expires_at"), // When pinning expires (2+ years from pinnedAt)
  publishStatus: publishStatusEnum("publish_status").default("draft"),
  publishedAt: timestamp("published_at"),
  provider: text("provider").notNull(), // pinata, web3storage, or infura
  metadata: jsonb("metadata"), // Additional project metadata (tags, category, etc.)
...
// Insert schemas for Project Uploads
export const insertProjectUploadSchema = createInsertSchema(projectUploads).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertProjectUpload = z.infer<typeof insertProjectUploadSchema>;
export type ProjectUpload = typeof projectUploads.$inferSelect;
